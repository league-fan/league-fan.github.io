...
import {
  getChampionByName,
  getSkinsOfChampionById,
  getSkinsOfSkinline,
  getSkinlineById,
  getSkinlinesOfUniverse,
  getUniverseById,
} from "@/data/helpers";
...
export default function SkinGridPage({
  patch,
  lng,
  champions,
  skins,
  ...
  if (!type || !allowType.includes(type))
    return (
      <NotFound title="Type not found" lng={lng} back={`/${lng}/champions`} />
    );
  ...
  if (!id || !validId(type, id))
    return (
      <NotFound title="Id not found" lng={lng} back={`/${lng}/champions`} />
    );
  ...
  if (type === "skinline") {
    ...
    const skinlineUniverse =
      universes.filter((u) => u.skinSets.includes(skinline.id)) ?? [];
      ...
      <div className={styles.container}>
        <FooterContainer>
          <div>
            ...
            <main>
              <SkinFromSkinline
                skinlineSkins={skinlineSkins}
                lng={lng}
                skinline={skinline}
                skinlineUniverse={skinlineUniverse}
              />
            </main>
          </div>
    ...
    if (universeSkinlines.length === 0)
      return (
        <NotFound
          title="Skinlines not found"
          lng={lng}
          back={`/${lng}/universes`}
        />
      );
    ...
    for (const skinline of universeSkinlines) {
      universeSkinlineSkins[skinline.id] = getSkinsOfSkinline(
        skinline.id,
        skins,
        champions,
      );
    }
    const splash = getSkinsOfSkinline(
      universeSkinlines[0].id,
      skins,
      champions,
    );
    return (
          ...
          <div>
            ...
            <main>
              <SkinFromUniverse
                universe={universe}
                lng={lng}
                universeSkinlineSkins={universeSkinlineSkins}
                universeSkinlines={universeSkinlines}
              />
            </main>
          </div>
          <Footer patch={patch} flat />
