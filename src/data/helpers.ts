...
export function getSkinsOfSkinline(
  skinline_id: number,
  skins: Skin[],
  champions: Champion[],
) {
...
export function getSkinlinesOfUniverse(
  universe: Universe,
  skinlines: Skinline[],
) {
  return skinlines.filter((line) => universe.skinSets.includes(line.id));
}
...
export function getChampionByName(name: string, champions: Champion[]) {
  return champions.find(
    (champ) => champ.alias.toLowerCase() === name.toLowerCase(),
  );
}
