  ...
  const filteredChamps = useMemo(() => {
    ...
    if (sort) {
      sortedChamps = champions.slice().sort((a, b) => {
        ...
        if (sort === "name")
          return a.name.localeCompare(b.name, languageZoneToBCP47[lng]);
        if (sort === "name_rev")
          return b.name.localeCompare(a.name, languageZoneToBCP47[lng]);
        if (sort === "alias")
          return a.alias.localeCompare(b.alias, languageZoneToBCP47[lng]);
        if (sort === "alias_rev")
          return b.alias.localeCompare(a.alias, languageZoneToBCP47[lng]);
        return 0;
      });
    }
