...
export default function SkinIdPage({
  ...
  const validId = (type: string, id: string) => {
    ...
    if (type === "skinline")
      return skinSkinlines.some((s) => s.id.toString() === id);
    return false;
  };
  ...
  const skinSkinline = skinSkinlines.find(
    (s) => s.id.toString() === id,
  ) as Skinline;
  const collectionName =
    type === "champion" ? skinChamp.name : skinSkinline.name;
  const collectionIcon = type === "champion" ? <User /> : <Folder />;
  const backTo = `/${lng}/skins?type=${type}&id=${id}`;
  const linkTo = (skin: Skin) =>
    `/${lng}/skins/${skin.id}?type=${type}&id=${id}`;
  const skinCollection =
    type === "champion" ? skinChampSkins : skinSkinlineSkins[id];
